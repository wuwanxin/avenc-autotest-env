...
proces = []
#run_bash = 0
import sevc_driver as process_env
...
def proc(instance,queueInfo,run_bash_id):
    ...
    if not os.path.exists(p2):
        while 1:
            ...
            #time.sleep(2)
    os.chdir(p2)
    shutil.copyfile(run_bash_dir + "/host", "host")
    cmd = "cp -r " + run_bash_dir + "/model . "
    os.system(cmd)
    ...
    #根据c模型生成的文件自定义
    #这些都是生成在当前运行目录下的文件
    out_files = ["bash.log","recon.yuv","output.mp4"]
    os.chdir(org_cwd)
    ...
    mp4_path=instance.mkdir_in_workspace("mp4")
    if os.path.exists(run_bash_dir+"/"+p2+"/output.mp4"):
        shutil.copyfile(run_bash_dir+"/"+p2+"/output.mp4",mp4_path+"/"+bs_path.split("/")[-1]+".mp4")
    #删除env
...
def prepare_one(instance,yuv_manager,run_queue,env_pool):
    id = 0
    max_wait_times=2
    ok=1
    ...
    if end_flag != 1:
        ...
        cmds = process_env.load_cmdline(co)
        print "[ModifyDefaultCfg]down.",file_path
        bs_path = cmds["-o"]
        print bs_path
        bash_title = cmds["title"]
        env_id = env_pool.get_env()
...
if __name__ == "__main__":
    ...
    # 1:open envronment
    instance = process_env.open_env(platform, cwd,set_space_name_suffix="",bash_id=run_bash,print_only=0)
    ...
    while 1:
        prepare_one(instance,yuv_manager,run_queue,_env_pool)
