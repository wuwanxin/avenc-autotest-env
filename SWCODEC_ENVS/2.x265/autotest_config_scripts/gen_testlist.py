...
codec = ["hevc"]
c_model_dir = "../c_model/"
cmdline_title_arr = ["hevc_testenc_vc8000e"]
Persize = 0x9000000
...
def changeYuvFileInfo(cfg_line,src_type,case_info_dict,_codec,out_change_flag):
    ...
    ret = change_line(cfg_line, str(picx)+"x"+str(picy),"--input-res")
    ret = change_line(ret, framerate*2,"--keyint")
    ret = change_line(ret, framenum,"--frames")
    ret = change_line(ret, framerate,"--fps")
...
def changeYuvIDInfo(line, case_info_dict, id_ignore):
    ...
    yuvpath = case_info_dict["src_path"]
    ret = change_line(ret, yuvpath, "--input")
...
def changeYuvIOInfo(line, case_info_dict, output_path):
    ...
    ret = change_line(ret, yuvpath, "--input")
    ret = change_line(ret, output_path, "--output")
